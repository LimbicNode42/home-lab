...
test('dashboard shell exposes six accessible hash-backed tabs without the old Work tab', () => {
  assert.match(indexSource, /class="tab-list" role="tablist" aria-label="Dashboard sections"/);
  for (const id of ['overview', 'knowledge', 'blog-drafts', 'reports', 'investment-screener', 'diary-goals']) {
...
test('dashboard panels are grouped into overview, knowledge, reports, investment screener, and diary-goals homes', () => {
  ...
  assert.match(indexSource, /id="panel-knowledge"[\s\S]*id="docs-panel"[\s\S]*id="epics-panel"[\s\S]*id="panel-blog-drafts"/);
  assert.match(indexSource, /id="panel-blog-drafts"[\s\S]*id="writing-panel"[\s\S]*id="panel-reports"[\s\S]*id="finnick-panel"[\s\S]*id="panel-investment-screener"/);
